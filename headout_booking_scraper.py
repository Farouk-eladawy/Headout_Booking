            ...
            try:
                while page_index < pages_limit and (limit is None or len(results) < limit):
                    ...
                    if on_batch_processed and current_page_bookings:
                        ...
                        # MEMORY FIX: Clear results if we are processing in batches 
                        # to prevent memory bloating over long periods.
                        # We only keep results if we aren't using the batch callback.
                        results.clear() 
                ...
                try:
                    ...
                    import logging
                    logging.getLogger("scraper_debug").error(f"Error closing context: {e}")
    ...
    async def run_and_sync(self, pages_limit: int = 2, start_page: int = 0, limit: int = 20, on_page_complete: Optional[Any] = None) -> List[Dict[str, Any]]:
        ...
        async def process_batch(batch: List[Dict[str, Any]]):
            ...
            for b in batch:
                ...
                date_changed = False # Default unless proven otherwise
                ...
                if existing and existing.get('synced_to_airtable'):
                    ...
                    # Check if DATE specifically changed (Experience Date OR Time Slot)
                    # If Headout date/time changes, we MUST update Airtable (overwrite manual edits)
                    old_date = norm(existing.get('experience_date'))
                    new_date = norm(b.get('experience_date'))
                    old_time = norm(existing.get('time_slot'))
                    new_time = norm(b.get('time_slot'))
                    
                    if old_date != new_date or old_time != new_time:
                        date_changed = True
                        # logger.info(f"Date change detected for {b['booking_id']}: {old_date} -> {new_date}")

                    is_same = (
                        norm(existing.get('status')) == norm(b.get('status')) and
                        not date_changed and
                    ...
                    # New booking or not synced yet -> Force date update (it's the first time!)
                    date_changed = True
                ...
                if needs_airtable_sync:
                    res = self.airtable.upsert_booking(b, force_date_update=date_changed)
                    if res.get("success"):
