...
class HeadoutBookingScraper:
    ...
    async def _augment_contact_details(self, page: Page, row_index: int, booking: Dict[str, Any]) -> None:
        ...
        # SKIP if cancelled
        # EDIT: User reported that even cancelled bookings might have contact info visible in row text
        # (e.g. Email: christian.micas@orange.fr is visible in the row in screenshot)
        # So we should NOT skip extraction if we can find it in the row text without clicking.
        # But we should probably skip CLICKING if status is cancelled, as the button might be disabled or missing.
        ...
        async with async_playwright() as p:
            ...
            while page_index < pages_limit and (limit is None or len(results) < limit):
                ...
                try:
                    ...
                    cnt = await nav.count()
                    if cnt > 0:
                        # Find next button or page number
                        # Logic: Click the button that corresponds to page_index + 1
                        # Note: Pagination logic might need refinement depending on UI structure
                        # Usually "Next" button is safer if available
                        # But here we stick to existing logic for now, just adding wait
                        
                        # We need to find the "Next" button specifically to avoid clicking "Last Page"
                        # Or rely on index.
                        # If we are on page X, we want X+1.
                        # Since we don't know the exact DOM, we try the existing logic which seemed to work:
                        # await nav.nth(min(page_index + 1, cnt - 1)).click() 
                        # BUT `page_index` here is relative to the loop (0..limit). 
                        # The DOM pagination buttons likely don't change index 0..limit.
                        # They likely show [1] [2] [3] ... [Next]
                        
                        # Better generic "Next" click:
                        # Try finding a button with > icon or "Next" text, or just the last button if it's not disabled
                        # Let's trust the previous logic for now but apply it carefully
                        # Or better: check for `aria-label="Go to next page"` or similar
                        
                        next_btn = page.locator('button[aria-label="Go to next page"], button[aria-label="Next page"], button[title="Next page"]')
                        if await next_btn.count() > 0:
                            await next_btn.first.click()
                        else:
                            # Fallback 1: Look for button with chevron or text
                            # SVG chevron often used. Or text ">"
                            # We can also check buttons that are NOT disabled.
                            ...
                            if await next_text_btn.count() > 0:
                                ...
                                # Fallback 2: The original logic which is risky but might be the only way if aria-labels missing
                                # However, we must ensure we don't click the SAME button if page_index resets.
                                # The original logic was: nav.nth(min(page_index + 1, cnt - 1))
                                # This assumes the pagination bar grows or shifts.
                                # If we are deep in pagination, it might be: [Prev] [10] [11] [12] [Next]
                                # We want the LAST button usually (Next).
    ...
    async def run_and_sync(self, pages_limit: int = 2, start_page: int = 0, limit: int = 20, on_page_complete: Optional[Any] = None) -> List[Dict[str, Any]]:
        ...
        async def process_batch(batch: List[Dict[str, Any]]):
            ...
            for b in batch:
                ...
                needs_airtable_sync = True
                if existing and existing.get('synced_to_airtable'):
                    # 2. Compare critical fields
                    # If fields match, we can skip Airtable update to avoid spamming edit history
                    # We normalize None to "" for comparison
                    def norm(v): return str(v or "").strip()
                    
                    is_same = (
                        norm(existing.get('status')) == norm(b.get('status')) and
                        norm(existing.get('experience_date')) == norm(b.get('experience_date')) and
                        norm(existing.get('time_slot')) == norm(b.get('time_slot')) and
                        norm(existing.get('pax_details')) == norm(b.get('pax_details')) and
                        norm(existing.get('customer_name')) == norm(b.get('customer_name')) and
                        # Only compare contact info if we actually found new contact info
                        (not b.get('customer_email') or norm(existing.get('customer_email')) == norm(b.get('customer_email'))) and
                        (not b.get('customer_phone') or norm(existing.get('customer_phone')) == norm(b.get('customer_phone')))
                    )
                    
                    if is_same:
                        needs_airtable_sync = False
                        # logger.info(f"Skipping Airtable sync for {b['booking_id']} (No changes).")
                
                # Always save to DB (it handles ON CONFLICT updates efficiently)
                self.db.save_booking(b)
                
                if needs_airtable_sync:
                    res = self.airtable.upsert_booking(b)
                    if res.get("success"):
                        self.db.mark_synced(b.get("booking_id"), res.get("recordid"))
                # Small delay per item to be safe
